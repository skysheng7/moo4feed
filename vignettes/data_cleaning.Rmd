...
## 8. Advanced Cleaning: Detect Outliers with KNN 🔍

> **Ollie says:** Even after our regular quality checks, there might still be unusual data points in our dataset. Let's use a fancy technique called K-Nearest Neighbors (KNN) to find and handle those tricky outliers!
...
These functions are designed to be able to handle both single-day and multi-day data. 

### Try KNN outlier detection

```{r knn-outliers}
# First, let's detect outliers in our feed data
...
# We can also visualize the outliers
p1 <- viz_outliers(
  feed_with_outliers,
  x_var = "duration",
  y_var = "intake",
...
### Process water data outliers

```{r knn-water-outliers}
# Now let's detect outliers in our water data
...
> **Ollie's Warning:** Be careful with removing outliers! What looks like an outlier might actually be important data. Always try to understand why the points are being flagged as outliers before removing them.

## 9. Summarize Daily Intakes 📊
...
Stay tuned for the next vignette (Part 2) where we'll explore what these dairy cows are actually up to in "Extracting Cow Stories"!
