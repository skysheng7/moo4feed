...
## Introduction to Meal Clustering

Ollie the Otter, our data processing specialist, explains: "Individual feeding visits represent only part of the behavioral story. Animals often take brief breaks during sustained feeding periods. Meal clustering groups related visits into meaningful feeding events."

Understanding feeding behavior requires grouping individual visits into meals - periods of sustained feeding activity separated by longer intervals. This analysis reveals natural feeding patterns and biological rhythms.
...
cat("Animals in day 1:", length(unique(clean_feed[[1]]$cow)), "\n")
```

## Problems with Fixed Meal Intervals

Traditional studies often use arbitrary fixed meal intervals (60 or 90 minutes) without customization to different datasets collected in different environments. This approach fails to account for differences among farms, individual animals and studies.

### Demonstration: Fixed 90-Minute Interval

```{r fixed-interval-demo, fig.width=8, fig.height=5}
# Apply fixed 90-minute interval clustering
set.seed(123)  # For reproducible random selection
selected_animal <- sample(unique(clean_feed[[1]]$cow), 1)
selected_date <- names(clean_feed)[1]
...
fixed_labeled <- meal_label_visits(
  data = clean_feed[[1]](which(clean_feed[[1]]$cow == selected_animal),),
  eps = 90,
  min_pts = 2,
  lower_bound = NULL,
  upper_bound = NULL,
...
## Data-Driven Meal Interval Selection

### Method 1: Percentile-Based Selection

The percentile method calculates gaps between consecutive visits for each animal within each day. By default we take a high percentile (default 93rd) as the threshold. Visits separated by less than this threshold belong to the same meal.
...
### Visualizing GMM Method

```{r viz-gmm, fig.width=8, fig.height=5}
...
### Comparing Data-Driven vs. Fixed Intervals

```{r compare-methods}
cat("Method comparison:\n")
cat("Fixed interval (traditional): 60 or 90 minutes\n")
cat("Percentile method:", round(eps_percentile, 2), "minutes\n")
cat("GMM method:", round(eps_gmm, 2), "minutes\n")
```

The data-driven approaches allows you to visualize and set the threshold customized to YOUR data.

Now let's cluster the data using the GMM method and visualize the results.

## DBSCAN Clustering Algorithm

### How DBSCAN Works

DBSCAN is like looking for "busy neighborhoods of visits":
...
### Different eps Calculation Scopes
...
cat("Meals identified by eps scope:\n")
...
```{r compare-clustering, fig.width=10, fig.height=8}
...
meal_plots[[1]][[1]]  # First animal, first day
```

### Individual Animal Analysis

Extract plots for a specific animal on a specific date:
