...
> - **Automatic vs Custom scaling**:
>   - Set `custom_scaling = NULL` to use automatic min-max scaling,
>     which normalizes all variables (duration, intake, rate) to a 0-1
>     range. This ensures equal contribution of each variable to the
>     distance calculation.
>   - You can also provide a list like
>     `custom_scaling = list(rate = 10000, intake = 7, duration = 0.03)`
>     to use custom scaling factors that give different weights to each
>     variable
> - **Why scaling?**: The KNN algorithm uses distance to identify
>   outliers. Variables at different scales (e.g., duration in seconds
>   vs. rate in kg/s) need to be scaled to ensure they contribute
