#' Create a time sequence from start to end, by seconds or minutes
#'
#' @description
#' Creates a sequence of timestamps from the earliest start time 
#' to the latest end time in the provided data. Default resolution is seconds,
#' but can be changed to minutes for memory efficiency.
#'
...
#' @return A POSIXct vector of time sequence at specified resolution
#' 
#' @examples
#' # Create toy data
#' toy_data <- data.frame(
...
#' )
#' 
#' # Create time sequence by seconds (default)
#' time_seq_sec <- create_time_sequence(toy_data, start_col = "start", end_col = "end")
#' length(time_seq_sec) 
#' 
#' # Create time sequence by minutes
#' time_seq_min <- create_time_sequence(toy_data, start_col = "start", end_col = "end", resolution = "min")
#' length(time_seq_min) 
#' 
#' @noRd
create_time_sequence <- function(cur_data, 
  ...
  # Validate and match resolution argument
  ...
  # Handle NA values and infinite values
  if (is.na(total_start) || is.na(total_end) || is.infinite(total_start) || is.infinite(total_end)) {
    stop("Start/End times cannot be NA or infinite", call. = FALSE)
  }
  
  # Validate that end time is not earlier than start time
  if (total_end < total_start) {
    stop("End time cannot be earlier than start time", call. = FALSE)
  }
  
  seq(total_start, total_end, by = resolution)
}
...
#' @inheritParams set_global_cols
#' @return Data frame: first column 'Time', others are animal IDs, values inside are initialized to 0
#' 
#' @examples
...
prepare_time_animal_matrix <- function(cur_data, 
  ...
  if (!lubridate::is.POSIXct(dateTime_seq)) stop("`dateTime_seq` must be date time data type: POSIXct")
...
#' Create an empty time-bin matrix (simple pass-through)
#'
#' @description
#' Simple function that returns the input matrix unchanged. Used for consistency
#' with old API structure.
#'
#' @param animal_time_matrix Data frame with Time column and animal columns
#' @return Input matrix unchanged
#' 
#' @keywords internal
#' @noRd
prepare_time_bin_matrix <- function(animal_time_matrix) {
  # Input validation
  if (!is.data.frame(animal_time_matrix)) {
    stop("`animal_time_matrix` must be a data frame", call. = FALSE)
  }
  
  if (!"Time" %in% names(animal_time_matrix)) {
    stop("`animal_time_matrix` must have a 'Time' column", call. = FALSE)
  }
  
  if (ncol(animal_time_matrix) < 2) {
    stop("`animal_time_matrix` must have at least one animal column", call. = FALSE)
  }
  
  return(animal_time_matrix)
}