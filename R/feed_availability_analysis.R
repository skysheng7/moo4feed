...
#'
#' Visits that occur before any feed addition to that bin will have NA values.
#'
#' @examples
...
calculate_feed_availability <- function(visit_data,
  ...
  visit_results <- lapply(names(visit_data), function(day_name) {
    ...
    # If no feed events, return visits with NA values
    if (nrow(feed_events) == 0) {
      if (nrow(visits) == 0) {
    ...
    # Using vectorized join approach instead of rowwise
